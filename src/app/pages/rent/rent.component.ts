...
export class RentComponent implements OnDestroy {
 ...
 enviar(){
  this.bookingService.saveBooking(this.vehicle!._id, this.form.value.fechaInicio,this.form.value.fechaFin ,this.numDias*this.vehicle!.pricePerDay, 0 ).subscribe({
    next:()=>{
      Swal.fire({
        title: "Reserva realizada",
        text: `tu ${this.vehicle?.brand} ${this.vehicle?.model} esta listo`,
        icon: "success",
        timer:2000,
        didClose: ()=> {
